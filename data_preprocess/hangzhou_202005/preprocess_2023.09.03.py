...
selet = np.where(p_num > np.sort(p_num, axis=0)[-101], True, False)
...
num_list = []
vec_list = []
for i in range(len(matrix_all),4,-1):
    num = matrix_all[i-4:i]
    vec = vec_maxtrix[i-4:i]
    sum_num = num.sum(axis = 0)
    sum_num[sum_num == 0] = 1
    vec[i,...,:256] = (num*vec[...,:256]).sum(axis = 0)/ sum_num
    vec[i,..., 256:512] = (num * vec[..., 256:512]).sum(axis=0) / sum_num
    vec[i,..., 256:512] = (num * vec[..., 256:512]).sum(axis=0) / sum_num
    num = num.sum(axis = 0)
    num_list.append(num)
    vec_list.append(vec)
...
read_path = os.path.join(DATA_ROOT,"line_graph_data")
...
# get_random_dataloader(1, 1, 2, 0.8, 0.2, read_path, save_path, GRID_NUM,if_vec=False, hour_num = hour_num)
