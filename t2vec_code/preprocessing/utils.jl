...
#     print(df.values.shape)
    ...
    println("DataFrame 的行数：", rows)
    println("DataFrame 的列数：", columns)
    df = df[df.MISSING_DATA .== false, :]
    sort!(df, [:TIMESTAMP])
    println("Processing $(size(df, 1)) trips...")
    ## writing in numeric matrix with hdf5
    h5open("E:/WST_code/zhijiang_data/porto_data/porto/porto/porto.h5", "w") do f
        num, num_incompleted = 0, 0
        print(length(df.POLYLINE))
        for i in range(1,stop=length(df.POLYLINE))
            trip = df.POLYLINE[i]
            try
