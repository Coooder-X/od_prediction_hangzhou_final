...
class ResidualGraphLSTM(nn.Module):
    def __init__(self, node_num,num_filters, network_structure_e, network_structure_l, network_structure_d,
        ...
        if self.use_three_branch:
            ...
            self.emb_vec_fusion = nn.Sequential(
                nn.Linear(self.network_structure_vec_e[-1] + self.network_structure_e[-1] + self.latent_dim_l,
                          self.network_structure_e[-1] + self.latent_dim_l),nn.ReLU())
            self.vec_lstm_structure = copy.deepcopy(self.network_structure_l)
            self.vec_lstm_structure[0] = self.vec_dim
            self.vec_lstm_structure[-1] = self.network_structure_vec_e[0]


            # 构建用于处理轨迹特征的LSTM并将其用于车辆轨迹预测
            self.vec_lstm_encoded = nn.ModuleList([])
            for l in range(1, len(self.vec_lstm_structure)):
                self.vec_lstm_encoded.append(nn.LSTM(self.vec_lstm_structure[l - 1], self.vec_lstm_structure[l], batch_first=True))
            # self.vec_lstm =
            # 构建用于处理轨迹特征的图网络
            ...
            self.emb_vec_fusion = nn.Sequential(
                nn.Linear(self.network_structure_e[-1] + self.latent_dim_l,
                          self.network_structure_e[-1] + self.latent_dim_l), nn.ReLU())
        ...
        self.Linear_3 = nn.Sequential(nn.Linear(self.network_structure_e[-1] + self.latent_dim_l, self.network_structure_d[0]), nn.ReLU())
        # 构建解码器
    ...
    def forward(self, X_input, graph_conv_filters_input, vec= None):
        ...
        if self.use_three_branch:
            vec = vec.reshape(vec.shape[0] * vec.shape[1], vec.shape[2], vec.shape[3])
            for i in range(0, len(self.network_structure_l) - 1):
                vec, (vec_emb, _) = self.vec_lstm_encoded[i](vec)
            vec_emb = nn.ReLU()(vec_emb.reshape(-1,self.node_num,vec_emb.shape[-1]))
        ...
        l_encoded = self.Linear_2(l_encoded).squeeze(0).unsqueeze(1).repeat(1,self.node_num,1)
        if self.use_three_branch:
            graph_decode = self.emb_vec_fusion(torch.cat((graph_encoded, graph_vec_encoded, l_encoded), dim=-1))
        else:
            graph_decode = self.emb_vec_fusion(torch.cat((graph_encoded, l_encoded), dim=-1))
